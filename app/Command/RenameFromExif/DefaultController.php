...
namespace App\Command\RenameFromExif;
...
class DefaultController extends CommandController
{
    ...
    public function handle()
    {
        ...
        foreach ($originalFiles as $file) {
            ...
            $output = exif_read_data($originalPath);
            if (false === $output || !isset($output['DateTimeOriginal'])) {
                $this->fileWithErrors[] = $originalPath;
                continue;
            }

            $date = $output['DateTimeOriginal'];
            $destinationPath = $destinationFolder . date('Ymd-His', strtotime($date)) . '.' . $extension;
